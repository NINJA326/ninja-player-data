...
function doGet(e) {
  ...
  try {
    ...
    else if (action === 'listPlayers' || action === 'players') result = listPlayersCached_();
    ...
    else if (action === 'setup') { setupSheets_(); clearAppCaches_(); result = {status:'ok',app:'NINJA PLAYER DATA v41',setup:true}; }
    else result = {status:'ok',app:'NINJA PLAYER DATA v41'};
  } catch (err) {
...
function clearAppCaches_() {
  try { CacheService.getScriptCache().remove('players_v27'); } catch (e) {}
}
function listPlayersCached_() {
  const cache = CacheService.getScriptCache();
  const cached = cache.get('players_v27');
  if (cached) { try { return JSON.parse(cached); } catch (e) {} }
  const result = listPlayers_();
  try { cache.put('players_v27', JSON.stringify(result), 300); } catch (e) {}
  return result;
}
...
function growthRecords_(playerId) {
  ...
  return { status:'ok', playerId:String(player.playerId||''), player, records };
}
